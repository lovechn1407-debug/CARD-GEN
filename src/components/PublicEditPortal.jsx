...
export default function PublicEditPortal() {
  ...
  const selectMemberToEdit = (found) => {
              ...
              <div style={{ marginTop: '16px', paddingTop: '14px', borderTop: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <div style={{ maxHeight: '260px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {matchingMembers.map((m) => (
                    <div
                      key={m.id}
                      onClick={() => selectMemberToEdit(m)}
                      style={{ padding: '10px 12px', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', transition: 'all 0.15s ease' }}
                      ...
                      <div>
                        ...
                        {m.phone && (
                          <div style={{ fontSize: '11px', color: '#16a34a', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '3px', marginTop: '2px' }}>
                            <Phone style={{ width: 11, height: 11 }} /> {m.phone}
                          </div>
                        )}
                        <div style={{ fontSize: '10px', color: '#1d4ed8', fontFamily: 'monospace', marginTop: '2px' }}>ID: {m.collegeRollNo}</div>
                      </div>

                      <span style={{ padding: '6px 12px', background: '#1d4ed8', color: '#fff', borderRadius: '6px', fontSize: '11px', fontWeight: 700, whiteSpace: 'nowrap' }}>
                        Edit Card →
                      </span>
                    </div>
                  ))}
